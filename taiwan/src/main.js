...
// Region view (e.g. Taiwan) while they're there; whole-trip view before they get there, in the air, and after they leave.
...
// "Where in the world is Cal?" for one traveller, "…are A & B?" for more.
const plural1 = /&|\band\b|,/.test(TRIP.travellers);
$('who').textContent = TRIP.travellers;
$('isAre').textContent = plural1 ? 'are' : 'is';
document.title = `Where ${plural1 ? 'are' : 'is'} ${TRIP.travellers}?`;
$('regionBtn').title = $('regionBtn').ariaLabel = TRIP.region.name;
$('regionBtn').querySelector('span').textContent = TRIP.region.flag;
