...
test('departure times', () => {
  const { rows } = parseCSV('Date,Country,City,Depart,Arrive\nSat 03 Oct,NZ > Singapore,Wellington > Changi,6:05am,21:30\nSun 04 Oct,Singapore,Changi,,', Date.UTC(2026, 8, 24));
  const trip = buildTrip(rows);
  assert.equal(trip.departures.get(Date.UTC(2026, 9, 3)), 365);
  assert.equal(trip.departures.size, 1);
});
