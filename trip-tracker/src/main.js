import './style.css';
import csv from './data/itinerary.csv?raw';
...
if (!rows.length) {
  ...
  // Country shapes are bundled (no CDN fetch), but loaded after first paint to keep startup quick.
  Promise.all([import('topojson-client'), import('world-atlas/countries-50m.json')]).then(([topo, atlas]) => {
    const w = atlas.default || atlas;
    world = topo.feature(w, w.objects.countries).features.filter(f => f.properties.name !== 'Antarctica');
    drawMap();
  });
}
